          ...
          p.values.forEach(function (d) {
            if (d.values.exp || d.values.imp)
              p.years.push({
                ...
                imp: d.values.imp,
                balance:
                  (d.values.exp - d.values.imp) /
                    (d.values.exp + d.values.imp) || 0,
                pct_exp: d.values.exp
                  ? (d.values.exp / indexYears[d.key].exp) * 100
                  : null,
                pct_imp: d.values.imp
                  ? (d.values.imp / indexYears[d.key].imp) * 100
                  : null,
                pct_tot:
                  d.values.exp || d.values.imp
                    ? ((d.values.exp + d.values.imp) / indexYears[d.key].tot) *
                      100
                    : null,
              });
          });
