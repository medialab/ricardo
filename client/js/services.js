      ...
      return {
        getReportingEntities: function (params) {
          ...
          $http({
            ...
            .error(function () {
              deferred.reject(
                "Error 500 : An error occured while fetching data"
              );
            });
          return deferred.promise;
        },
  ...
  .factory("cfSource", function () {
    var cf = crossfilter([]),
      ...
      continents = continent
        ...
        .reduce(
          reduceAddContinent,
          reduceRemoveContinent,
          reduceInitialContinent
        )
        .order(order),
  ...
  .factory("utils", function () {
    ...
    function keysIndex(a) {
      ...
      for (i = 0, l = a.length; i < l; i++)
        for (k in a[i]) if (!~keys.indexOf(k)) keys.push(k);
    ...
    function toCSVString(data, params) {
      ...
      if (!header.length)
        if (plainObject && params.headers !== false) header = keys;
        ...
        .map(function (row) {
          return row
            .map(function (item) {
              ...
              ).replace(new RegExp(rescape(escape), "g"), escape + escape);

              // Escaping if needed
              return ~i.indexOf(delimiter) ||
                ~i.indexOf(escape) ||
                ~i.indexOf("\n")
                ? escape + i + escape
                : i;
