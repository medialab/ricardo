    ...
    function (fileService, apiService, $timeout) {
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          var height = 400,
            width = document.querySelector("#linechart-world-container")
              .offsetWidth,
            sort = [],
            yValue = "total",
            duration = 500;
          ...
          function linechart(data, yValue, minDate, maxDate) {
            ...
            x.domain([
              new Date(minDate - 1, 0, 1),
              new Date(maxDate + 1, 0, 1),
            ]);
            y.domain([0, yMax]);
            ...
            function mouseover(d) {
              if (d !== undefined) {
                if (d[yValue] !== null && d[yValue] !== undefined) {
                  var colorPoint =
                    scope.view === "world"
                      ? colorLine(d.reporting_id)
                      : colorLine(d.partner_id);
                  focus.attr(
                    "transform",
                    "translate(" +
