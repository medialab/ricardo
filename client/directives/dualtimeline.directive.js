    ...
    function ($timeout) {
      return {
        ...
        link: function (scope, element, attrs) {
          scope.$watchCollection("[ngData, endDate, startDate]", function (
            newValue,
            oldValue
          ) {
            if (newValue[0]) {
          ...
          function draw(data) {
            ...
            var margin = { top: 20, right: 0, bottom: 30, left: 0 },
              width =
                document.querySelector("#dual-timeline-container").offsetWidth -
                margin.left -
                margin.right,
                ...
                data.filter(function (d, i) {
                  if (d.imp !== null) {
                    if (i === 0) {
                      ...
                      if (data[i - 1].imp === null && data[i + 1].imp === null)
                        return d;
                    }
                  }
                })
                ...
                data.filter(function (d, i) {
                  if (d.exp !== null) {
                    if (i === 0) {
                      ...
                      if (data[i - 1].exp === null && data[i + 1].exp === null)
                        return d;
                    }
                  }
                })
            ...
            if (
              svg.select("g.x.axis").empty() ||
              svg.select("g.y.axis").empty()
            ) {
              gx = svg
                .append("g")
                .attr("class", "x axis")
