...
angular
  .module("ricardo.directives.reportingWorld", [])

  /* directive with watch, update and draw functions */
  .directive("reportingWorld", [
    function () {
      return {
        restrict: "E",
        ...
        link: function (scope, element, attrs) {
          scope.$watchCollection(
            "[ngData,startDate,endDate,flowType,layout]",
          ...
          var margin = { top: 20, right: 0, bottom: 40, left: 0 },
            width =
              document.querySelector("#reporting-world-container").offsetWidth -
              margin.left -
              margin.right,
            height = 300,
            offsetHeight = 10,
          ...
          var tip_margin = { top: 20, right: 0, bottom: 10, left: 0 },
            tip_width =
              document.querySelector(".continent-tooltip").offsetWidth -
              tip_margin.left -
              tip_margin.right;
          ...
          var svg_tip = tooltip
            ...
            .attr(
              "transform",
              "translate(" + tip_margin.left + "," + tip_margin.top + ")"
            );
          ...
          function draw(data) {
            ...
            x.domain([
              new Date(scope.startDate - 1, 0, 1),
              new Date(scope.endDate + 1, 0, 1),
            ]);
            // y.domain([0,maxFlow])

            var dataFiltered = data_flatten.filter(function (d) {
              return (
                d.year >= x.domain()[0].getFullYear() &&
                d.year <= x.domain()[1].getFullYear()
              );
            });
            ...
            if (layout === "multiple") {
              ...
              area.y0(height / partners).y1(function (d) {
                return y(d[yValue]);
              });
            ...
            if (layout === "single") {
              ...
              if (svg.select("g").empty()) {
                var multi_g = svg.selectAll(".multiple").data(data);
                multi_g
                  .enter()
                  .append("g")
                  .attr("class", "multiple")
                ...
                .on("mousemove", function () {
                  ...
                  if (selectData.length > 0) {
                    selectData.sort(function (a, b) {
                      return b.values[yValue] - a.values[yValue];
                    });
                    tooltip
                      .select(".title")
                      .html(
                    ...
                    var tip_partner = tooltip
                      ...
                      .attr("transform", function (d, i) {
                        return (
                          "translate(0," + 2 * i * (offsetHeight + 2) + ")"
                        );
                      });
                    tip_partner
                      .append("rect")
                      .attr("width", function (d) {
                        return d.values[yValue] !== null
                          ? x_tip(d.values[yValue])
                          : 0;
                      })
                      .attr("height", 10)
                      .attr("fill", function (d) {
                        return partnerColors(d.partner);
                      });
                    tip_partner
                      .append("text")
                      .text(function (d) {
                    ...
                    tip_partner
                      ...
                      .text(function (d) {
                        return d[yValue] !== null
                          ? format(Math.round(d.values[yValue])) + " £"
                          : "N/A";
                      })
                      .attr("x", function (d) {
                        return d.values[yValue] !== null
                          ? x_tip(d.values[yValue]) + 2
                          : 2;
                      })
                      .attr("y", 9)
