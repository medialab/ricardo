 ...
 */
angular
  .module("ricardo.controllers.country", [])
  ...
  .controller("country", [
    ...
    function (
      ...
      if (localStorage.sourceEntitySelected) {
        try {
          $scope.entities.sourceEntity.selected = JSON.parse(
            localStorage.getItem("sourceEntitySelected")
          );
          init($scope.entities.sourceEntity.selected.RICid, $scope.currency);
        } catch (e) {
          $scope.entities.sourceEntity.selected = $scope.reportingEntities.filter(
            function (e) {
        ...
        apiService
          ...
          .then(function (data) {
            ...
            data.RICentities.partners.forEach(function (d) {
              ...
              if (
                $scope.RICentitiesDD[
                  $scope.RICentitiesDD.length - 1
                ].RICname.indexOf("World ") !== 0
              )
                $scope.RICentitiesDD[$scope.RICentitiesDD.length - 1].RICname =
                  d.RICname + "[" + d.type + "]";
            });
            ...
            $scope.reportingCountryEntities = $scope.RICentitiesDD.filter(
              function (d) {
                return d.type === "country";
              }
            );
            $scope.reportingColonialEntities = $scope.RICentitiesDD.filter(
              function (d) {
                return d.type === "colonial_area";
              }
            );
            $scope.reportingGeoEntities = $scope.RICentitiesDD.filter(function (
              d
            ) {
              return (
                d.type === "geographical_area" &&
                d.RICname.indexOf("World ") !== 0
              );
            });
            $scope.reportingWorldEntities = $scope.RICentitiesDD.filter(
              function (d) {
                return (
                  d.type === "geographical_area" &&
                  d.RICname.indexOf("World ") === 0
            ...
            $scope.reportingCountryEntities1 = $scope.RICentitiesDD.filter(
              function (d) {
            ...
            $scope.rawYearsRange = d3.range(
              $scope.rawMinDate,
              $scope.rawMaxDate + 1
            );
            $scope.rawYearsRange_forInf = d3.range(
              $scope.rawMinDate,
              $scope.selectedMaxDate
            );
            $scope.rawYearsRange_forSup = d3.range(
              $scope.selectedMinDate + 1,
              $scope.rawMaxDate + 1
            );
            /*
            ...
            localStorage.setItem(
              "sourceEntitySelected",
              JSON.stringify($scope.entities.sourceEntity.selected)
            );
          });
      }
       ...
       */
      $scope.$watchCollection("[selectedMinDate, selectedMaxDate]", function (
        newValue,
        oldValue
      ) {
      ...
      $scope.$watch("entities.sourceEntity.selected", function (
        newValue,
        oldValue
      ) {
      ...
      function updateTableData() {
        ...
        var tableData = $scope.tableData.filter(function (d) {
          return (
            d.year >= $scope.selectedMinDate && d.year <= $scope.selectedMaxDate
          );
        });
        ...
        if (partner_selected.length > 0) {
          ...
          $scope.reporting.forEach(function (r) {
            if (r.type !== "continent")
              ...
              var dataFiltered = tableData.filter(function (d) {
                return (
                  $scope.RICentities["" + d.partner_id].continent === r.RICid
                );
              });
      ...
      $scope.pushReporting = function (elm) {
        ...
        initLinechart(
          $scope.reporting,
          $scope.linechartFlow.type.value,
          $scope.linechartCurrency.type.value
        );
        updateTableData();
      };
      ...
      function initPartnerHisto(data) {
        ...
        temp.forEach(function (d) {
          if (
            d.year >= $scope.selectedMinDate &&
            d.year <= $scope.selectedMaxDate
          ) {
            data.push(d);
          }
        });
            ...
            if (d.type !== "continent") {
              apiService
                ...
                .then(function (result) {
                  ...
                  //   $scope.selectedMaxDate)
                  yearSelected = initTabLineChart(
                    result,
                    yearSelected,
                    d.type,
                    d.RICid
                  );
                  var linechartData = initLineChart2(
                    linechart_flows,
                    yearSelected,
                    d.RICid,
                    yValue,
                  ...
                  if (linechartData.length === partners.length)
