...
angular.module("ricardo.controllers.bilateral", []).controller("bilateral", [
  ...
  function (
    ...
    try {
      ...
      apiService
        ...
        .then(function (result) {
          $scope.partnerEntities = result.RICentities.partners.filter(function (
            d
          ) {
            return (
          ...
          if ($scope.partnerEntities.length === 0)
          ...
          .then(
            function (data) {
              ...
              localStorage.setItem(
                "selectedMinDate",
                JSON.stringify($scope.selectedMinDate)
              );
              localStorage.setItem(
                "selectedMaxDate",
                JSON.stringify($scope.selectedMaxDate)
              );
              ...
              if (res[1] === 500) {
                ...
                if (
                  ...
                  $scope.message =
                    ...
                    $scope.entities.targetEntity.selected.RICname;
                } else {
                  $scope.message =
                    "Missing Target " +
                    $scope.entities.targetEntity.selected.RICname;
      ...
      if (newValue !== oldValue && newValue) {
        ...
        if ($scope.entities.targetEntity.selected !== undefined) {
          apiService
            ...
            .then(function (result) {
              $scope.partnerEntities = result.RICentities.partners.filter(
                function (d) {
                  return (
                    RICids.indexOf(d.RICid) !== -1 &&
                    d.type === "country" &&
    ...
    $scope.$watch("entities.targetEntity.selected", function (
      newValue,
      oldValue
    ) {
      if (newValue !== oldValue && newValue) {
    ...
    function updateDateRange() {
      ...
      if ($scope.allData !== undefined) {
        ...
        $scope.tableData = cfSource
          .year()
          .top(Infinity)
          .concat(cfTarget.year().top(Infinity));
