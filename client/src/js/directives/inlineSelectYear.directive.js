/* Directives */
angular
  .module("ricardo.directives.inlineSelectYear", [])

  /* directive with only watch */
  .directive("inlineSelectYear", [
    function () {
      return {
        restrict: "E",
        ...
        link: function (scope, element, attrs) {
          if (scope.rawModel && scope.rawList) {
            scope.model = {
              name: "" + scope.rawModel,
              value: Number(scope.rawModel),
            };
            scope.list = scope.rawList.map(function (d) {
              return { name: "" + d, value: Number(d) };
            });
          }
